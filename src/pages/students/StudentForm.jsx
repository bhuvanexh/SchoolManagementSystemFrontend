...
const StudentForm = () => {
  ...
  useEffect(() => {
    if (isEdit && current?._id === id) {
      reset({
        ...current,
        classId: current.section?.classId || '',
    ...
    <PageWrapper>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <FormSection title="Student Details">
          ...
          <FormField control={control} name="parentContact" label="Parent Contact" error={errors.parentContact} />
          <FormField control={control} name="email" label="Email" type="email" error={errors.email} />
          {role === 'admin' ? <FormField control={control} name="classId" label="Class" type="select" options={buildOptions(classes)} error={errors.classId} /> : null}
          <FormField control={control} name="sectionId" label="Section" type="select" options={buildOptions(sections)} error={errors.sectionId} disabled={role === 'teacher'} />
