  ...
  return (
    <PageWrapper>
      <PageHeader
        ...
        actions={role !== 'student' && canCreateNoticeOrEvent ? <Link to="/notices/new"><PrimaryButton><span className="inline-flex items-center gap-2"><PlusCircle className="h-4 w-4" /> Create Notice</span></PrimaryButton></Link> : null}
      />
              ...
              <article key={notice._id} className="glass-panel p-6">
                <div className="flex items-start justify-between gap-3">
                  <button
                    ...
                    className="text-left"
                  >
                    <h2 className="text-xl font-bold text-on-surface hover:text-primary hover:underline">{notice.title}</h2>
                    <p className="mt-2 text-sm text-on-surface-variant">{formatDate(notice.createdAt)}</p>
                  </button>
                  <div className="flex items-center gap-2">
                    <Tooltip text="View details"><button type="button" onClick={() => navigate(`/notices/${notice._id}`)} className="rounded-full bg-white p-2 text-primary"><Eye className="h-4 w-4" /></button></Tooltip>
